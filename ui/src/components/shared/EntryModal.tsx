...
export default function EntryModal({ date, entries, onClose, onSave }: EntryModalProps) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!editingEntry) {
        ...
        if (existingSexEntry && addSex) {
          throw new Error('Sex entry already exists for this date');
        }
      ...
      >
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <form id="entry-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <fieldset className="border-t pt-4">
              <legend className="text-lg font-semibold text-gray-700 mb-4 flex justify-between items-center">
                <span>Intimacy</span>
                ...
                <div id="existing-sex-entries" className="space-y-4 mb-4">
                  {entries
                    .filter(e => e.date.split('T')[0] === date.split('T')[0] && e.entry_type === 'sex')
                    .map((entry, index) => (
                      <div 
                        key={entry.id} 
                            ...
                            <button
                              ...
                              onClick={async () => {
                                if (confirm('Delete this entry?')) {
                                  try {
                                    const result = await deleteEntry('sex', entry.id!.toString());
                                    if (result.success) {
        ...
        <div className="p-6 border-t">
          <button
            ...
            className="w-full bg-pink-600 text-white font-bold py-3 px-6 rounded-lg shadow-md hover:bg-pink-700 transition disabled:opacity-50"
          >
            {isSubmitting ? 'Saving...' : 'Save'}
          </button>
          {message && (
            <div className={`text-sm mt-2 ${message.includes('Error') || message.includes('Failed') ? 'text-red-600' : 'text-green-600'}`}>
              {message}
            </div>
          )}
        </div>
