...
import { registerServiceWorker, setupPWAInstallPrompt, installPWA, requestNotificationPermission, subscribeToPushNotifications } from '@/utils/serviceWorker';
...
export default function ServiceWorkerRegistration() {
  ...
  useEffect(() => {
    ...
    console.log('🚀 ServiceWorkerRegistration component mounted, starting registration...');
    ...
    if (typeof window !== 'undefined') {
      ...
      if (!standaloneDetected) {
        if (iosDetected) {
          ...
          // Also listen for custom PWA installable event
  ...
  const handleNotificationRequest = async () => {
    try {
      ...
      // If permission granted, also subscribe to push notifications
  ...
  return (
    <>
      ...
      {/* Notification Permission Banner - All Browsers */}
        ...
        <div className="bg-gradient-to-r from-blue-500 to-teal-600 text-white p-3 rounded-lg mb-4 shadow-lg">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => setNotificationPermission('denied' as NotificationPermission)}
      ...
      {isIOSSafari && !isStandalone && (
        <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white p-3 rounded-lg mb-4 shadow-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <span className="text-2xl">�</span>
              <div>
                <p className="text-sm font-medium">Install Cuddles App</p>
                <p className="text-xs text-blue-100">Get the full PWA experience with notifications and offline access</p>
              </div>
            </div>
            <button
              onClick={() => setShowIOSGuidance(true)}
              className="bg-white text-purple-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-100 transition-colors"
            >
              Install App
            </button>
