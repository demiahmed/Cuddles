...
export async function loadMonthlySexStats(): Promise<Array<{ month: string; count: number }>> {
  try {
    const response = await fetch(`/api/stats/sex/monthly`);
    if (!response.ok)
      throw new Error(
        `Failed to fetch monthly sex stats: ${response.status} ${response.statusText}`
      );
    return await response.json();
  } catch (err) {
    console.error("Error loading monthly sex stats:", err instanceof Error ? err.message : err);
    return [];
  }
}